---
layout: layouts/base
title: Uses
permalink: /uses/
---

<div class="post-card">

# Uses

The apps, gear, and services I use day to day. Part of the [uses.tech](https://uses.tech) community.

## Hardware

- 💻 **Computer**: MacBook Pro 14" - M3 Pro - 36GB Memory
- 📱 **Phone**: iPhone 16 Pro Max
- ⌚ **Watch**: Analog watch from Watzmann
- 🎧 **Headphones**: AirPods Max (at home); AirPods 4th gen
- ⌨️ **Keyboard**: Keychron K4
- 🖱️ **Mouse / Trackpad**: Logitech MX Master 3
- 🖥️ **Monitor**: Lenovo ThinkVision 34" Ultrawide Curved Monitor
- 🪑 **Desk**: Boho Office Basic Line with a self made desk plate

## Development

- 💻 **Terminal**: [Ghostty](https://ghostty.org/) / [Superset](https://superset.sh)
- 📝 **Editor**: nvim
- 🔀 **Version Control**: Git

## This Site

- 🏗️ **Static Site Generator**: [Eleventy (11ty)](https://www.11ty.dev/)
- 🎨 **Theme**: Catppuccin
- 📡 **Hosting**: [Hetzner](https://www.hetzner.com)
- 🔄 **Deploy**: GitHub Actions via FTP
- 🌐 **Social & Profile**: [omg.lol](https://home.omg.lol/referred-by/flohgro)

## Productivity

- 📝 **Notes**: [Drafts](https://getdrafts.com) + [Craft](https://www.craft.do/)
- ✅ **To-Do**: [Todoist](https://todoist.com)
- 🗓️ **Calendar**: [Fantastical](https://flexibits.com/fantastical)
- ⌨️ **Launcher**: [Raycast](https://www.raycast.com/hey/081e003e)
- 📁 **File Management**: [DEVONthink](https://www.devontechnologies.com/apps/devonthink) + Finder
- 🔖 **Bookmarks**: [DEVONthink](https://www.devontechnologies.com/apps/devonthink)
- 👓 **Read It Later**: now [Quick Reads](https://quickreads.app) coming from [Readwise Reader](https://readwise.io/i/floh)
- 📰 **RSS**: now [Quick Reads](https://quickreads.app) coming from [Readwise Reader](https://readwise.io/i/floh)
- 🛝 **Presentations**: [Deckset](https://www.deckset.com)

## Communication

- ✉️ **Mail Client**: Apple Mail
- 📨 **Mail Server**: iCloud + [Hetzner](https://www.hetzner.com)
- 💬 **Chat**: iMessage, WhatsApp, [Signal](https://signal.org)
- 🐘 **Mastodon**: [Ivory](https://tapbots.com/ivory/)

## Media

- 🎶 **Music**: Apple Music
- 🎧 **Podcasts**: [Castro](https://www.castro.fm)
- 📷 **Photo Shooting**: Camera, [Halide](https://halide.cam/)
- 📚 **Photo Management**: Apple Photos
- 🤳 **Photo Editing**: [Photomator](https://www.pixelmator.com/photomator/) (iOS) · [Pixelmator Pro](https://www.pixelmator.com/pro/) (Mac)

## System & Utilities

- 🌐 **Browser**: Safari
- 🔐 **Password Manager**: [1Password](https://1password.com)
- 🗄️ **Cloud Storage**: iCloud Drive · Nextcloud · OneDrive
- 🌤️ **Weather**: [Carrot Weather](https://www.meetcarrot.com/weather/)
- 👨🏻‍💻 **AI Assistant**: [Claude](https://claude.ai/)

## Home & Finance

- 💰 **Budgeting**: [MoneyMoney](https://moneymoney-app.com)
- 🛒 **Shopping Lists**: Shared list in Apple Reminders
- 🧑‍🍳 **Meal Planning**: [Cookidoo](https://cookidoo.thermomix.com/foundation/en-US)

</div>

{% include "random-quote.njk" %}
